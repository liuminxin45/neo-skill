...
## Degrees of freedom
This skill is designed for **medium** freedom (high=heuristic, low=deterministic scripts).

## Requirement collection (at most 10 questions)
Ask only what is missing. Never exceed 10 questions total.
1. 你的仓库形态是什么（pnpm workspace / pnpm+nx / pnpm+turbo / 单包），以及现有命令习惯（lint/typecheck/test/depcheck）是什么？
2. 语言/框架范围是什么（TS/JS/React/Vue/Node），是否存在必须兼容的既有 ESLint/TS 配置？
3. public entrypoints 的口径是否固定为 package.json#exports（是否允许例外/迁移期白名单）？
4. 边界检查工具倾向哪条路径（eslint-plugin-boundaries / dependency-cruiser），是否允许新增 devDependencies？
5. 分层模型是否采用 type-first（presentation/application/domain/infra/shared/types/tests），是否需要按仓库实际命名调整？
6. 复杂度门槛是否沿用默认：单函数 60 行、圈复杂度 15、鼓励早返回减少嵌套（保留必要 try-catch）？
7. 错误处理与日志：是否要求统一错误类型并限制 console.* 仅在边界层（entry/infra）？
8. CI PR gate 需要覆盖哪些命令集合（最小门禁 vs 全量门禁），以及是否需要 affected/增量优化（nx/turbo）？
...
---
Generated by ai-skill-creator at 2026-01-21T15:00:59Z.
