...
## 目标
- 通过仓库形态识别，自动配置可执行的技术门禁（ESLint/TypeScript/Prettier/边界/依赖检查）
- 确保所有规则可自动化、失败即阻断（fail-fast）
...
规则模板：
- canonical：`skills/hard-gate/assets/windsurf-workflow-data/repo-shapes/*.json`
- Windsurf workflow data：`.windsurf/workflows/data/hard-gate/repo-shapes/*.json`

## 基于 git diff 的主要改动层级识别（type-first）
默认层级模型模板：
- canonical：`skills/hard-gate/assets/windsurf-workflow-data/layers/type-first.json`
- Windsurf workflow data：`.windsurf/workflows/data/hard-gate/layers/type-first.json`
